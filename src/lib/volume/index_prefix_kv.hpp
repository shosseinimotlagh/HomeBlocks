...
public:
    ...
    VolumeIndexKey(lba_t k) {
        m_lba_base = static_cast< uint32_t >(k >> 32);
        m_lba_offset = static_cast< uint32_t >(k & 0xFFFFFFFF);
    }
    ...
    int distance(BtreeKey const& f) const override {
        VolumeIndexKey const& from = static_cast< VolumeIndexKey const& >(f);
    ...
    sisl::blob serialize_prefix() const override {
        return sisl::blob{reinterpret_cast< uint8_t* >(const_cast< uint32_t* >(&m_lba_base)),
                          static_cast< uint32_t >(sizeof(uint32_t))};
    }

    sisl::blob serialize_suffix() const override {
        return sisl::blob{reinterpret_cast< uint8_t* >(const_cast< uint32_t* >(&m_lba_offset)),
                          static_cast< uint32_t >(sizeof(uint32_t))};
    }
    ...
    void deserialize(sisl::blob const& prefix, sisl::blob const& suffix, bool) {
        ...
        uint32_t const* other_p = reinterpret_cast< uint32_t const* >(prefix.cbytes());
        m_lba_base = *other_p;

        uint32_t const* other_s = reinterpret_cast< uint32_t const* >(suffix.cbytes());
    ...
    lba_t key() const { return (static_cast< uint64_t >(m_lba_base) << 32) | m_lba_offset; }
...
private:
#pragma pack(1)
    // Store blkid and checksum as the value. Most significant 32 bits of blk_id contains chunk_num
...
public:
    VolumeIndexValue(const blk_id& base_blkid, homestore::csum_t csum) : homestore::BtreeIntervalValue() {
        m_blkid_suffix = static_cast< uint32_t >(base_blkid.to_integer() & 0xFFFFFFFF) >> 1;
        m_blkid_prefix = static_cast< uint32_t >(base_blkid.to_integer() >> 32);
        m_checksum = csum;
    }
    ...
    homestore::blk_id blkid() const {
        ...
        return blk_id{blk_num, nblks, chunk_num};
    }
    ...
    void shift(int n, void* app_ctx) override {
        auto ctx = reinterpret_cast< IndexValueContext* >(app_ctx);
    ...
    sisl::blob serialize_prefix() const override {
        return sisl::blob{reinterpret_cast< uint8_t* >(const_cast< uint32_t* >(&m_blkid_prefix)),
                          static_cast< uint32_t >(sizeof(uint32_t))};
    }
    sisl::blob serialize_suffix() const override {
        // Include both m_blkid_suffix and checksum in the suffix.
        return sisl::blob{reinterpret_cast< uint8_t* >(const_cast< uint32_t* >(&m_blkid_suffix)),
                          static_cast< uint32_t >(sizeof(uint32_t) + sizeof(homestore::csum_t))};
    }
    uint32_t serialized_prefix_size() const override { return static_cast< uint32_t >(sizeof(uint32_t)); }
    uint32_t serialized_suffix_size() const override {
        return static_cast< uint32_t >(sizeof(uint32_t) + sizeof(homestore::csum_t));
    }
    ...
    void deserialize(sisl::blob const& prefix, sisl::blob const& suffix, bool) override {
        ...
        m_blkid_prefix = *(reinterpret_cast< uint32_t const* >(prefix.cbytes()));
        m_blkid_suffix = *(reinterpret_cast< uint32_t const* >(suffix.cbytes()));
        m_checksum = *(reinterpret_cast< homestore::csum_t const* >(suffix.cbytes() + sizeof(uint32_t)));
    }
